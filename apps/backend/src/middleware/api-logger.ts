import { createLogsService } from "@/features/logs/service";
...
export function apiLoggingMiddleware(): MiddlewareHandler {
  return async (c, next) => {
    ...
    // Clone request for later processing (but don't parse yet)
    const contentType = c.req.header("content-type");
    let clonedRequest: Request | null = null;

    if (method !== "GET" && method !== "HEAD" && contentType) {
      // Clone the request to avoid consuming the stream, but defer parsing
      clonedRequest = c.req.raw.clone() as unknown as Request;
    }

    // Store minimal response data to defer processing
    ...
    try {
      ...
      // Capture minimal response data (no parsing yet)
      ...
      // Clone the response for later processing (but don't parse yet)
      clonedResponse = c.res.clone();
    } catch (error: any) {
      ...
      // Defer all request and response body processing until waitUntil
      c.executionCtx.waitUntil(
        (async () => {
          let requestBody: any = null;
          let responseBody: any = null;

          // Process request body asynchronously
          if (clonedRequest && contentType) {
            try {
          ...
          if (clonedResponse && responseContentType) {
          ...
          // Now log with the processed request and response bodies
          await ApiLogger.log(logsService, {
            method,
            url,
            path,
            userAgent,
