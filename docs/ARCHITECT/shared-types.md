...
| ~~Document error responses + status-narrow derivations~~ | ✅ | `2026-05-24:` Shipped `a95460d`. Logs routes document error statuses (401/500, +404 on `/{id}`) with `httpErrorResponseSchema` content (`{ message, stack? }`, matching stoker's `onError` body); `contracts.ts` status-narrows every derivation to the success status (200, or 201 for `/test`) so the `.data` peel stays clean. Zod-validation (422) errors are intentionally left undocumented |
